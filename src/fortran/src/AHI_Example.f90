...
!	! Test vals
	...
	do
		...
		if (ncmd>= 3) nchans	=	nchans + 1
		read( inarg, '(i10)',iostat=ios )retval
		if (ncmd>= 3 .and. ios /= 0) nchans	=	nchans -1
		ncmd	=	ncmd + 1
	enddo
